...
- **온라인**: 이 저장소를 GitHub Pages 등으로 배포한 경우, 배포 URL에서 볼 수 있습니다.  
  - 프로젝트 사이트 예: `https://<org 또는 user>.github.io/docs/`  
...
- **배포**: `main` 브랜치 push 시 GitHub Actions로 빌드 후 **GitHub Pages**에 배포됩니다.  
  **Settings → Pages → Source**를 **GitHub Actions**로 선택해야 합니다.  
  프로젝트 페이지(예: `/docs/`)로 배포할 때는 Actions Variables에 `BASE_URL=/docs/` 설정.  
  ...
  **"Your Docusaurus site did not load properly" / baseUrl 경고가 나오면**: 접속 주소가 `username.github.io/docs/` 형태(프로젝트 페이지)인데 빌드 시 `baseUrl`이 `/`이면 발생합니다. **Settings → Secrets and variables → Actions → Variables**에서 `BASE_URL` = `/docs/` 를 추가한 뒤 워크플로를 다시 실행하세요.
